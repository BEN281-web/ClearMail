            ...
            {showProfile && (
              <div className={`absolute right-0 top-10 w-56 rounded-lg border ${t.card} shadow-xl`}> 
                ...
                <div className="p-2 text-sm">
                  ...
                  <button onClick={()=>{ try { localStorage.removeItem("clearmail_history"); window.dispatchEvent(new Event("clearmail_history_updated")) } catch(error) {console.log(error);}; setShowProfile(false) }} className={`w-full text-left px-3 py-2 rounded-md hover:brightness-110`}>Clear history</button>
...
function useHistoryData() {
  ...
  return { items, refresh: () => {
    try { const data = JSON.parse(localStorage.getItem("clearmail_history") || "[]"); setItems(data) } catch(error) {console.log(error);}
  } }
}
